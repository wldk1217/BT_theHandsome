...
<script type="text/javascript">
			...
			.ready(
					function() {
						...
						$(document).on(
								...
								".pageBtn",
								function() {
									GA_Event('카테고리_리스트', '페이지',
											categoryListList.PARAMS.pageNum);
								});
						...
						categoryListList = new BoardListController(
								'categoryListForm', '#listBody', '.paging', 12); //폼아이디, 리스트공간 셀렉터, 페이징공간 셀렉터, 페이지당 리스트개수
											...
											function(index) {
												...
												if (this.productStyleSizeTemp != undefined) {
													...
													if (!Object.keys) {//IE8 이하
																						...
																						.each(
																								...
																								function(
																									...
																									if (value != undefined) {
																										...
																										var siteTempArr = value[0]
																												.split(',');

																										var siteTempNewArr = [];

																										for (var i = siteTempArr.length - 1, j = 0; i >= 0; i--, j++) {
																											siteTempNewArr[j] = siteTempArr[i]
																													.trim();
																										}

																										siteTempNewArr = siteTempNewArr
																												.reduce(
																														function(
																																a,
																																b) {
																										...
																										for (var i = 0; i < siteTempNewArr.length; i++) {
																											for (var j = i; j < siteTempNewArr.length; j++) {
																												var sizeArr = [];
																												var aSiteTmpSize = siteTempNewArr[i]
																														.trim()
																														.toUpperCase();
																												var bSiteTmpSize = siteTempNewArr[j]
																														.trim()
																														.toUpperCase();
																												...
																												if (orderSizeCodeArr[aSiteTmpSize] == null
																														|| orderSizeCodeArr[aSiteTmpSize] == "") {
																													var tmpSize = "1";
																													var sizeStr = siteTempNewArr[i]
																															.trim();
																													for (var k = 0; k < (9 - sizeStr.length); k++) {
																														tmpSize += "0";
																													}
																													aSizeSortVal = tmpSize
																															+ sizeStr;
																												} else {
																												...
																												if (orderSizeCodeArr[bSiteTmpSize] == null
																														|| orderSizeCodeArr[bSiteTmpSize] == "") {
																													var tmpSize = "1";
																													var sizeStr = siteTempNewArr[j]
																															.trim();
																													for (var k = 0; k < (9 - sizeStr.length); k++) {
																														tmpSize += "0";
																													}
																													bSizeSortVal = tmpSize
																															+ sizeStr;
																												} else {
														...
														if (Object
																.keys(this.productStyleSizeTemp).length > 0) {
															var keyLength = Object
																	.keys(this.productStyleSizeTemp).length;
															$
																	.each(
																			this.productStyleSizeTemp,
																			function(
												...
												if (this.productFlag != undefined) {
													var productFlagTemp = this.productFlag
															.split(',');
												}

												if (imageUrl == ""
														&& "http://cdn.thehandsome.com/_ui/desktop/common/images/products/no_img3.jpg" != this.productImageUrl1) {
													imageUrl = this.productImageUrl1
															.substring(
																	0,
												...
												if (this.productImageUrl1
														.indexOf(this.productStyleCode) < 0) {
													if (this.productImageUrl1 != null
															&& this.productImageUrl1 != "") {
														var tempArrCode = this.productImageUrl1
																.split("/");
														if (tempArrCode.length > 1) {
															var tempTargetCode = tempArrCode[tempArrCode.length - 1];
															var tempArrTargetCode = tempTargetCode
																	.split("_");
															this.productStyleCode = tempArrTargetCode[0]
																	+ "_"
																	+ tempArrTargetCode[1];
														}
													}
												...
												if ("" != "OU") {
													...
													if (target_url
															.indexOf("/c/ou_") == -1) { //아울렛이 아니면
														if (target_url
																.indexOf("/c/") > -1) { //정상상품 리스트
															// if'/c/카테고리코드' : /가 한개면
															if (target_url
																	.substring(
																			target_url
																...
																if (target_url
																		...
																		.indexOf(
																				"br") > -1) {
																	gaProductFunction = "onclick=\"setEcommerceData('"
																			+ index
																			+ "', 'BRAND');\"";
																} else {
																	gaProductFunction = "onclick=\"setEcommerceData('"
												...
												listTag += "        <a href=\"/ko/p/"
														...
														+ "?categoryCode=we011\" class=\"item_info1\" "
														+ gaProductFunction
														+ "> \n";
												...
												listTag += "        <a href=\"/ko/p/"
														+ this.productStyleCode
														+ "?categoryCode=we011\" class=\"item_info2\" "
														+ gaProductFunction
														+ "> \n";
												//listTag += "     <a href=\"/ko/p/"+ this.productStyleCode +"\" class=\"item_info2\"> \n";
												listTag += "            <span class=\"brand\">"
												...
												if ("" != "OU") {
													if (productFlagTemp
														...
														if (Number(this.productNormalityPrice) > Number(this.productPrice)) {
															chkSaleYn = true;
															var productSalePerCent = getDecimalToString(
																	((this.productNormalityPrice - this.productPrice)
																			/ this.productNormalityPrice * 100),
																	1, 1);
															var productNormalityPriceTag = "<del class=\"del_price_201029\">￦"
																	+ addComma(this.productNormalityPrice)
																	+ "</del>";
															var productSalePerCentTag = "<span class=\"discount_rate_201029\">"
																	+ productSalePerCent
																	+ "%</span>";
														}
													}

													var productPriceTag = "<span "
															+ (productFlagTemp
																	.indexOf("SALE") > -1
																	&& chkSaleYn ? "class=\"ou_price\""
																	: "")
															+ ">￦"
															+ addComma(this.productPrice)
															+ "</span>";

													listTag += "            <span class=\"price\"><span id=\"price_"+this.productStyleCode +"\">"
															+ productPriceTag
															+ (productFlagTemp
																	.indexOf("SALE") > -1
																	&& chkSaleYn ? productNormalityPriceTag
																	+ productSalePerCentTag
																	: "")
															+ "</span></span> \n";
												} else if ("" == "OU") {
														...
														if (Number(this.productNormalityPrice) > Number(this.productPrice)) {
															var productSalePerCent = getDecimalToString(
																	((this.productNormalityPrice - this.productPrice)
																			/ this.productNormalityPrice * 100),
																	1, 1);
															var productOuPriceTag = "<span class=\"ou_price\">￦"
																	+ addComma(this.productPrice)
																	+ "</span>";
															var productNormalityPriceTag = "<del class=\"del_price_201029\">￦"
																	+ addComma(this.productNormalityPrice)
															...
															listTag += "            <span class=\"price\"><span id=\"price_"+this.productStyleCode +"\">"
																	+ productOuPriceTag
																	+ productNormalityPriceTag
																	+ productSalePerCentTag
																	+ "</span></span> \n";
														} else {
															listTag += "            <span class=\"price\"> <span id=\"price_"+this.productStyleCode+"\"><span>￦"
																	+ addComma(this.productPrice)
																	+ "</span></span></span> \n";
														}
													} else {
												...
												if (this.productAvailableYn == 0) {
													...
													if (this.stickerTextUseYn == true) {
														listTag += "                <span class=\"point-mark\" style=\"background-color:#"+this.stickerTextInfo.stickerColor+";color: white;\">"
																+ this.stickerTextInfo.stickerText
																+ "</span> \n";
														stickerTextYN = true;
													}
													...
													for (var i = 0; i < productFlagTemp.length; i++) {
														...
														if (productFlagTemp[0] != ""
																|| hsDeliveryYn != "true") {

															if (!skipFlag) {
																listTag += "                <span class=\""+classTemp+"\">"
																		+ productFlagTemp[i]
																				.replace(
																						"RE_ORDER",
																						"재입고")
													...
													if (!this.productReserveSaleYn) {//예약상품이 아닐 경우
														if ((this.available4pmSum >= 1 && checkTime != "")
																|| (quickYn == "true" && checkQuickTime == true)
																|| (chkAthome == "ATHOME" && this.availableAtHome >= 1)) {
													...
													if (this.productReviewCount != "0"
															&& this.productReviewCount != null) {

														listTag += " <span class=\"review1902 ch1904\">"
												...
												if (!Object.keys) {//IE8 이하
													...
													if (keyLength.length > 0) {
														...
														$
																.each(
																		this.productColorTemp,
																		function(
																				key,
																				value) {
																			var colorChipArr = value
																					.split(";");
																			if (colorChipArr[1] != null
																					&& colorChipArr[1]
																							.indexOf("C01") > -1) {
																				colorChipImg = "background:"
																						+ colorChipArr[0]
																						+ " url('"
																						+ colorChipArr[1]
																						+ "/dims/resize/13x14')";
																			...
																			if (keyLength > 1) {
																				listTag += "    <a href=\"javascript:chgColorChip("
																						+ index
																						+ ", '"
																						+ key
																						+ "')\" class=\"cl wt\" style=\""
																						+ colorChipImg
																						+ ";\" onclick=\"GA_Event('카테고리_리스트','컬러칩','"
																						+ key
																								.split('_')[1]
																						+ "')\" ></a> \n";
																			} else if (keyLength == 1) {
																				listTag += "    <a href=\"javascript:void(0);\" class=\"cl wt\" style=\""
																						+ colorChipImg
																						+ ";\" onclick=\"GA_Event('카테고리_리스트','컬러칩','"
																						+ key
																								.split('_')[1]
													...
													if (Object
															.keys(this.productColorTemp).length > 0) {
														var keyLength = Object
																.keys(this.productColorTemp).length;
														var colorChipImg = "";
														listTag += "<div class=\"color_more_wrap\"> \n";
														$
																.each(
												...
												if (this.productWishlist != 0) {
													wishlistClass = "on";
													wishlistAdd = "javascript:addWishListClick('"
												...
												listTag += "     <a href=\""
														...
														+ "\" id=\"wish_"
														+ this.productBaseCode
														+ "\" onclick=\"GA_Category('wish', $(this));brazeLogCustomEvent('"
														+ index
														+ "');\" data-value='"
														+ this.productStyleCode
														+ "'>위시리스트 담기</a> \n";
						...
						categoryListList.afterSetAction = function() {
							...
							$('.cate_item4 > ul > li').each(
									function(cate_item4) {
										if ((cate_item4 % 4) == 0) {
											$(
													'.cate_item4 > ul > li:nth-child('
															+ (cate_item4 + 4)
															+ ')').addClass(
													'mr1m');
										}
							...
							$(".cate_item4 .mr1m").next("li").css("clear",
									"both");
							...
							$(".item_box img")
									.on(
											"error",
											function() {
												var image = "http://media.thehandsome.com/"
														+ $(this).attr(
																"targetCode")
														+ "_"
														+ $(this).attr("id")
												...
												if (checkImageUrl1 == $(this)
													...
													if (checkImageUrl2 == $(
														...
														$(this)
																.attr("src",
																		"http://cdn.thehandsome.com/_ui/desktop/common/images/products/no_img3.jpg");
														checkImageUrl1 = "";
														checkImageUrl2 = "";
													} else {
														$(this).attr("src",
						...
						function active(item) {
							var $item_box = $(item);
							$item_box.find('.item_size').stop().slideDown(
									'fast');
							$item_box.find('.item_img .on').stop().fadeIn(
									'fast');
						}
						function deactive(item) {
							var $item_box = $(item);
							$item_box.find('.item_size').stop().slideUp('fast');
							$item_box.find('.item_img .on').stop().fadeOut(
									'fast');
						}
	...
	function setRepProdColorCode(colorCode) {
		...
		$('#colorCurrent').remove();
		$(
				'<span class="current" id="colorCurrent" title="' + color + '">'
						+ color + '</span>').insertBefore(
				'.color > .select > .ico_arr');
	}
	...
	function setRepSizeEnumCode(sizeCode) {
		...
		$('#sizeCurrent').remove();
		$(
				'<span class="current" id="sizeCurrent" title="' + size + '">'
						+ size + '</span>').insertBefore(
				'.size > .select > .ico_arr');
	}
	...
	function setPriceOrderCode(o, priceCode) {
		...
		$('#priceCurrent').remove();
		$(
				'<span class="current" id="priceCurrent" title="' + price + '">'
						+ price + '</span>').insertBefore(
				'.price > .select > .ico_arr');
	}
...
<div id="bodyWrap" class="products">
	...
	<h3 class="cnts_title ou1804">
		<span> <!-- 정상 브랜드 카테고리 목록 --> <a
			href="/list/${clarge}/${cmedium}/${csmall}"
			onclick="GA_Event('카테고리_리스트','카테고리','여성')">
				${(clarge=="MEN")?"남성":"여성"}</a> <img
							...
							<div class="color_more_wrap">
								<c:forEach items="${colorMap[status.index] }" var="colorList"
									...
									<a href="javascript:void(0)" class="cl wt"
										style="background:${colorList.ccolorcode} url('${colorList.cimage1 }');"
