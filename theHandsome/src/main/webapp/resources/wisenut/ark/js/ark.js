    ...
    if (arkQuery.browser.opera || arkQuery.browser.mozilla) {
        ...
        arkQuery(document).keyup(function (event) {
            ...
            if (event.keyCode == 38 || event.keyCode == 40) {
                ...
                moveFocusEvent(event);
            } else if (event.keyCode == 16) {} else if (event.keyCode == 8 && query == "") {
...
function setArkLocation() {
    ...
    // 화살표 닫기 이미지 위치 설정
    arkQuery("#" + arkUpId).css({
        "top": (arkImgTop - queryHeight) + "px"
    });
    arkQuery("#" + arkUpId).css({
        "left": ((queryWidth - 10) + arkImgLeft) + "px"
    });

    // 화살표 열기 이미지 위치 설정
    arkQuery("#" + arkDownId).css({
        "top": (arkImgTop - queryHeight) + "px"
    });
    arkQuery("#" + arkDownId).css({
        "left": ((queryWidth - 10) + arkImgLeft) + "px"
    ...
    // 자동완성접기 툴팁 이미지 위치 설정
    arkQuery("#tooltip01").css({
        "top": (offsetTop + tooltip01TopPos) + "px",
        "left": (offsetLeft + queryWidth + tooltip01LeftPos) + "px"
    });
}
...
function setArkSize() {
    ...
    arkQuery("#" + contentListId).css({
        "width": (queryWidth + arkWidth) + "px"
    }); // 자동완성 결과 리스트 넓이 설정
    arkQuery("#" + footerId).css({
        "width": (queryWidth + arkWidth) + "px",
        "margin-left": "250px",
        "margin-top": "105px"
    }); // 자동완성 풋터 넓이 설정
